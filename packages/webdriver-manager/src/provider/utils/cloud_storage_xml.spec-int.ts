...
import {httpBaseUrl} from '../../../test/server/env';
...
describe('cloud_storage_xml', () => {
  ...
  describe('with a http server', () => {
    beforeAll(async () => {
      proc = spawnProcess('node', ['dist/test/server/http_server.js']);
    ...
    describe('updateXml', () => {
      ...
      const xmlUrl = httpBaseUrl + '/test/support/files/foo.xml';
      ...
      it('should request and write the file if it is expired', async () => {
        const mtime = new Date(Date.now() - (60 * 60 * 1000) - 5000);
        ...
        spyOn(fs, 'statSync').and.returnValue({size: 1000, mtime} as fs.Stats);
    ...
    describe('convertXmlToVersionList', () => {
      const fileName = 'test/support/files/chromedriver.xml';
      ...
      it('should return a null value if the file does not exist', () => {
        const versionList = convertXmlToVersionList(
            'test/support/files/does_not_exist.xml', '.zip',
